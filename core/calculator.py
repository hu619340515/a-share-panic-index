...
class PanicIndexCalculator:
    ...
    def standardize(self, series: pd.Series) -> pd.Series:
        ...
        # 处理NaN值，替换为0.5
        standardized = standardized.fillna(0.5)
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df["limit_std"] = (
            self.standardize(df["limit_ratio"]) if "limit_ratio" in df.columns else 0.5
        )
        df["basis_std"] = (
            self.standardize(df["futures_basis"])
    ...
    def get_signal(self, panic_index: float) -> Dict[str, str]:
        """根据恐慌指数生成交易信号"""
        if panic_index >= 80:
            return {
                "signal": "buy",
