    ...
    def fetch(self, start_date: datetime, end_date: datetime) -> Optional[pd.Series]:
        ...
        try:
            ...
            for i, year_month in enumerate(months):
                try:
                    payload = {"yearMonth": year_month, "pageIndex": 1, "pageSize": 100}
                    ...
                    if data.get("code") == 20000 and data.get("data", {}).get("list"):
                        ...
                        if i % 5 == 0 or i == len(months) - 1:
                            print(
                                f"      [{i+1}/{len(months)}] {year_month}: {len(list_data)}条"
                            )
            ...
            for item in sorted(all_data, key=lambda x: x["date"]):
                if item["date"] not in seen:
                    ...
                    item["limit_ratio"] = (
                        item["limit_down"] / total if total > 0 else 0.5
                    )
            ...
            return df['limit_ratio'] if 'limit_ratio' in df.columns else None
