...
class Visualizer:
    ...
    def __init__(self):
        ...
        try:
            ...
            # 使用默认字体
            self.chinese_font = fm.FontProperties()
        except Exception:
            # 如果失败，使用默认设置
            plt.rcParams["font.family"] = "sans-serif"
    ...
    def plot_comparison(
        self, df: pd.DataFrame, raw_data: dict, output_path: str = "panic_vs_index.png"
    ):
        """绘制恐慌指数与大盘指数对比图（双轴）"""
        ...
        ax1.set_ylabel(
            "恐慌指数", fontproperties=self.chinese_font, fontsize=12, color=color1
        )
        line1 = ax1.plot(
            df.index, df["panic_index"], linewidth=2.5, color=color1, label="恐慌指数"
        )
        ...
        # 填充恐慌区间
        ax1.fill_between(
            df.index, 0, self.thresholds.get("greedy", 20), alpha=0.2, color="#2ecc71"
        )
        ax1.fill_between(
            df.index,
            self.thresholds.get("greedy", 20),
        ...
        ax1.fill_between(
            df.index, self.thresholds.get("panic", 80), 100, alpha=0.2, color="#e74c3c"
        )
        ...
        for name, value in self.thresholds.items():
            if name in ["greedy", "optimistic", "neutral", "panic"]:
                ax1.axhline(
                    y=value, color="gray", linestyle="--", alpha=0.3, linewidth=0.8
                )
        ...
        plt.title(
            title_text, fontproperties=self.chinese_font, fontsize=14, fontweight="bold"
        )
    ...
    def plot_comprehensive(
        ...
        plt.savefig(
            output_path, dpi=150, bbox_inches="tight", facecolor="white", pad_inches=0.3
        )
    ...
    def plot_simple(self, df: pd.DataFrame, output_path: str = "panic_chart.png"):
        ...
        # 绘制恐慌指数
        ax.plot(
            df.index, df["panic_index"], linewidth=2, color="#1f77b4", label="恐慌指数"
        )
    ...
    def _plot_panic_vs_index(self, ax, df, x_min, x_max, raw_data=None):
        ...
        # 主轴 - 恐慌指数
        ax.set_ylabel(
            "恐慌指数", fontproperties=self.chinese_font, fontsize=11, color="#1f77b4"
        )
        ...
        for i, (label, color) in enumerate(
            [
                ...
                ("neutral", "#9b59b6"),
                ("panic", "#e67e22"),
                ("extreme_panic", "#e74c3c"),
            ]
        ):
            lower = list(self.thresholds.values())[i - 1] if i > 0 else 0
            upper = (
                list(self.thresholds.values())[i] if i < len(self.thresholds) else 100
            )
        ...
        # 上证指数 - 优先使用raw_data
        sh_data = (
            raw_data.get("sh_index") if "sh_index" in raw_data else df.get("sh_index")
        )
    ...
    def _plot_volatility(self, ax, df, x_min, x_max):
        """波动率"""
        if "iv" in df.columns:
            ax.plot(df.index, df["iv"], linewidth=1.5, color="#e74c3c")
            ax.fill_between(df.index, 0, df["iv"], alpha=0.3, color="#e74c3c")
            ax.set_ylabel("波动率 (%)", fontproperties=self.chinese_font, fontsize=10)
            ax.set_title(
                "沪深300 历史波动率", fontproperties=self.chinese_font, fontsize=12
            )
            ax.set_xlim(x_min, x_max)
            ax.grid(alpha=0.3)
    ...
    def _plot_limit_up_down(self, ax, df, x_min, x_max, raw_data=None):
        ...
        # 优先从raw_data获取涨跌停数据
        limit_up = (
            raw_data.get("limit_up") if "limit_up" in raw_data else df.get("limit_up")
        )
        limit_down = (
            raw_data.get("limit_down")
            if "limit_down" in raw_data
            else df.get("limit_down")
        )
        ...
        if limit_up is not None and limit_down is not None and len(limit_up) > 0:
            ...
            ax.set_title(
                "每日涨跌停家数", fontproperties=self.chinese_font, fontsize=12
            )
            ...
            if len(limit_up) > 0:
                ...
                latest_down = (
                    limit_down.iloc[-1] if latest_date in limit_down.index else 0
                )
                ax.annotate(
                    f"涨停: {int(latest_up)}\n跌停: {int(latest_down)}",
    ...
    def _plot_southbound(self, ax, df, x_min, x_max):
        ...
        if "southbound_flow" in df.columns:
            ...
            ax.axhline(y=0, color="black", linewidth=0.5)
            ax.set_ylabel(
                "净流入 (亿港元)", fontproperties=self.chinese_font, fontsize=10
            )
            ax.set_title(
                "南向资金 (港股通)", fontproperties=self.chinese_font, fontsize=12
            )
            ax.set_xlim(x_min, x_max)
            ax.grid(alpha=0.3, axis="y")
